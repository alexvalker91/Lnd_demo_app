...
class SecondPage(private val driver: AndroidDriver) : AbstractPage(driver) {

    @FindBy(id = "button")
    private val button: WebElement? = null

    @FindBy(xpath = "//android.widget.Toast[contains(@text, 'Success')]")
    private val toastMessage: WebElement? = null

    public fun clickButton(): SecondPage {
    ...
    public fun getTextFromToast(): String {
        WebDriverWait(driver, Duration.ofMillis(25_000))
            .until(ExpectedConditions.presenceOfElementLocated(xpath("//android.widget.Toast[contains(@text, 'Success')]")))
        val toastText: String = toastMessage?.text ?: ""
        System.err.println("Log: $toastText")
        System.err.println("Log: getEmulatorNetworkStatus = ${getEmulatorNetworkStatus()}")
        return toastText
    }
